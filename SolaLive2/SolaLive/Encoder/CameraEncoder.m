...
- (void)changeCameraStatus {
    ...
    int select = [[user objectForKey:OnlyAudio] intValue];
    if (select == 0) {
        if (inputs.count < 2) {
            AVCaptureDevice *newCamera = nil;
            AVCaptureDeviceInput *newInput = nil;
...
//    NSString *url = @"rtmp://www.easydss.com:10085/hls/SolaLive_3";
        ...
        if(connection == self.videoConnection) {
            if (_running) {
                dispatch_async(encodeQueue, ^{
                    ...
                    int select = [[user objectForKey:OnlyAudio] intValue];
                    if (select == 0) {
                        [h264Encoder encode:sampleBuffer size:_outputSize];
                    } else {
                        [h264Encoder encode:sampleBuffer size:_outputSize];
                    }
...
- (void) sendPacket:(unsigned int)u32AVFrameLen {
    ...
    if (!_timer) {
        ...
        dispatch_source_set_event_handler(_timer, ^{
            [self.delegate sendPacketFrameLength:sendFrameLength / 1];
            sendFrameLength = 0;
        });
